...
namespace Paint2API.Controllers
{
    ...
    public class QuantityColorsController : ControllerBase
    {
        ...
        [HttpGet("GetDataByColorID/{id}")]
        public async Task<ActionResult<QuantityColor>> GetDataByColorID(int? id)
        {
            if (_context.QuantityColors == null)
            {
                return NotFound();
            ...
            var quantityColor = await _context.QuantityColors
                ...
                .Include(a => a.WareHouses)
                .Include(a => a.WareHouses.City)
                .FirstOrDefaultAsync(a => a.Colors.IdColor == id);
    ...
    .SelectMany(
        ...
        [Authorize(Roles = "3")]
        [HttpDelete("DeleteQuantityColorByColorId/{id}")]
        public async Task<IActionResult> DeleteQuantityColorByColorId(int? id)
        {
            if (id == null)
            {
            ...
            var quantityColors = await _context.QuantityColors.Where(a => a.ColorId == id).ToListAsync();

            if (quantityColors == null || quantityColors.Count == 0)
            {
                return NotFound();
            }

            foreach (var quantityColor in quantityColors)
            {
                _context.QuantityColors.Remove(quantityColor);
            }
